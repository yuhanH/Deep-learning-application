...
def make_bar_plot(axes, texts, heights, width=0.8, colors=None, ylim = None):
    """
    ...
    if ylim is None:
        ylim = [min(0,min(heights)),  max(0,max(heights))]
...
def make_single_sequence_spectrum(axis, row, row_scores, one_hot_decoding=None, colors=None, ylim =None):
...
#        one_hot_decoding = ["A", "T", "C", "G"]
    ...
    make_bar_plot(axis, sequence, score_sequence, colors=color_sequence, ylim = ylim)
